...
# predict and recommend songs based on song's year
...
def get_song_features_by_names(song_name, df_song):
    ...
    return song_row[['year']].values  # returned as numpy arr
...
def recommend_songs(X, df_song, year, scaler, k=5):
    # user input processing / standardization
    user_input = np.array([[year]])
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            recommend_songs(X, df_song, user_song_features[0], scaler)
            break
